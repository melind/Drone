...
// On insert dans la base de donnée commandes l'id de l'user, le prix du drone, le code du drone, la couleur selectionner et le status de la commande pour CHAQUE produit dans le panier
	foreach ($_SESSION["panier_item"] as $item) {  // $item -> Item séléctionner dans le pannier
		...
		$ajout = $DB->insert('INSERT INTO commandes (id_user, code_drone, prix, couleur, statut, id_drone) VALUES (:id_user, :code_drone, :prix, :couleur, :statut, :id_drone)', array('id_user' => $id, 'code_drone' => $Code, 'prix' => $Prix, 'couleur' => $Couleur, 'statut' => 'Completed', 'id_drone' => $id_drone));
...
<body>
	<div class="payment">
		...
		<br>
		<p>Votre achat à bien été effectué.</p>
		<p>Vous allez être redirigé vers l'accueil.</p>
		<p>Si la redirection ne marche pas, cliquez <a href="../index.php">ici</a>.</p>
	</div>
