...
if(!empty($_POST))
{
	...
	{
		$id = $_SESSION['id_user'];
		$DB->update('UPDATE user SET adresse ="' . $Adresse . '", ville ="' . $Ville .'", code_postale ="' . $CodeP .'" WHERE id_user="' . $id . '"');
			...
			<fieldset class="modiflivraison">
				<legend> INFORMATIONS DE LIVRAISON </legend>
					...
					<div class="modif">
						...
						<input class="info_modif"  name="CodeP" placeholder="Code Postale" value="<?php if (isset($CodeP)) echo $CodeP; ?>" maxlength="20" required="required">
					</div>
