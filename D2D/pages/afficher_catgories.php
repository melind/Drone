...
if(!empty($_GET["action"])) {
	...
	switch($_GET["action"]) {
		...
		if(!empty($_POST["quantite"])) {
			...
			$itemArray = array($productByCode[0]["code"]=>array('nom_drone'=>$productByCode[0]["nom_drone"], 'code'=>$productByCode[0]["code"], 'quantite'=>$_POST["quantite"],'id_drone'=>$productByCode[0]["id_drone"], 'prix'=>$productByCode[0]["prix"]));//=panier
				...
				if(in_array($productByCode[0]["code"],$_SESSION["panier_item"])) {
					...
					foreach($_SESSION["panier_item"] as $k) {
						if($productByCode[0]["code"] == $k)
							$_SESSION["panier_item"][$k]["quantite"] = $_POST["quantite"];//nbr atcicle choisit lors de cette session = qtité choisit dans le tableau ùproducByCode
					}
					// Permet de pouvoir ajouter plusieur produit dans le panier, sans le else, on ne peut mettre qu'un seul produit dedans.
				} else {
					$_SESSION["panier_item"] = array_merge($_SESSION["panier_item"],$itemArray);//panier = à ce que l'on vient d'ajouter (=sessionpanier = Rien coisit)
				}
				// Permet de pouvoir ajouter 1 produit dans le panier, sans le else, on ne peut rien mettre dedans.
			} else {
				$_SESSION["panier_item"] = $itemArray;//panier = 0 car rien encore choisit
			}
		}
		break;
	}
...
<head>
	...
	<link rel="stylesheet" href="/D2D/categorie.css">

	<title>Catégories de drones</title>
</head>
<body>
<?php
...
if (isset($ip)){
	...
	if(!empty($_SESSION["panier_item"])){
		...
		$session_items = count($_SESSION["panier_item"]);//compte le nbre d'item différents
	}
	}
	...
	<div id="product-grid">
		...
		<?php
		$product_array = $DB->query("SELECT * FROM product WHERE id_cat =".$id);//recup donées par id
			...
			<div class="card">
				...
				<div class="card-body">
					...
					<p class="card-text">Poids : <?= $p['poids'] ?> g</p>
					 <div class="infos">
					<input type="number" name="quantite" value="1" size="2" />
					<br>
